...
describe('pg:copy', function () {
  ...
  context('heroku to heroku with additional credentials', function () {
    ...
    it('copies', async function () {
      ...
      expect(stderr.output).to.include('Warning: pg:copy will only copy your default credential and the data it has access to.')
      expect(stderr.output).to.include('Any additional credentials and data that only they can access will not be copied.')
      expect(stderr.output).to.include(copyingText)
    })
  })
