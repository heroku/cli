...
`)
    ...
    it('# errors out when already exists', async function () {
      ...
      const {error} = await runCommand(BuildpacksAdd, ['https://github.com/foobar/foobar', '-a', 'example'])
    ...
    it('# errors out on unmapped codon urls', async function () {
      ...
      const {error} = await runCommand(BuildpacksAdd, ['https://codon-buildpacks.s3.amazonaws.com/buildpacks/heroku/jvm-common.tgz', '-a', 'example'])
    ...
    it('# errors out when already exists urn', async function () {
      ...
      const {error} = await runCommand(BuildpacksAdd, ['heroku/ruby', '-a', 'example'])
...
`)
    ...
    it('# returns an error message when i is not an integer', async function () {
      const {error} = await runCommand(BuildpacksAdd, ['https://github.com/heroku/heroku-buildpack-ruby', '-i', 'notinteger', '-a', 'example'])
    ...
    it('# returns an error message when i < 0', async function () {
      const {error} = await runCommand(BuildpacksAdd, ['https://github.com/heroku/heroku-buildpack-ruby', '-i', '-1', '-a', 'example'])

      expect(error?.message).to.include('Invalid index. Must be greater than 0.')
    })

    it('# returns an error message when i == 0', async function () {
      const {error} = await runCommand(BuildpacksAdd, ['https://github.com/heroku/heroku-buildpack-ruby', '-i', '0', '-a', 'example'])
