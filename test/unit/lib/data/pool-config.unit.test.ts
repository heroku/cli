...
describe('PoolConfig', function () {
  ...
  beforeEach(function () {
    extendedLevelsInfo = levelsResponse.items.map(item => ({
      ...
      name: item.name,
      pricing: Object.entries(pricingResponse.advanced).find(([_, value]) => value.product_description === item.name)?.[1],
      vcpu: item.vcpu,
    }))
    promptStub = stub(PoolConfig.prototype, 'prompt')
  })
  ...
  describe('followerInteractiveConfig', function () {
    it('allows the user to configure a follower pool', async function () {
      ...
      expect(stderr).to.contain('A cluster can have up to 13 follower instances.')
      expect(stderr).to.contain('Configure Follower Pool ~$0.083/hour ($60/month)')
    })
  ...
  describe('leaderInteractiveConfig', function () {
    it('allows the user to configure a leader pool', async function () {
      ...
      expect(stderr).to.contain('The leader pool has high availability enabled and includes a standby instance for redundancy.')
      expect(stderr).to.contain('Configure Leader Pool ~$0.167/hour ($120/month)')
    })
