...
function* run (context, heroku) {
  ...
  function displayNotifications (notifications) {
    ...
    for (let n of notifications) {
      ...
      for (let followup of n.followup) cli.log(wrap(`${cli.color.gray.dim(time.ago(new Date(followup.created_at)))}\n${followup.body}`));
    }
  }

  if (context.flags.json) cli.styledJSON(notifications);
