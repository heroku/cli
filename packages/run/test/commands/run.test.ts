...
describe('run', () => {
  ...
  testFactory()
    .command(['_run', '--app=heroku-run-test-app', 'echo 1 2 3'])
    .it('runs a command', async ctx => {
      expect(ctx.stdout).to.include('1 2 3\n')
    })

  testFactory()
    .command(['_run', '--app=heroku-run-test-app', 'ruby -e "puts ARGV[0]" "{"foo": "bar"} " '])
  ...
  testFactory()
    .command(['_run', '--app=heroku-run-test-app', '-e FOO=bar', 'env'])
    .it('runs a command with env vars', ctx => {
      expect(ctx.stdout).to.include('FOO=bar')
    })

  testFactory()
    .command(['_run', '--app=heroku-run-test-app', '--exit-code', 'invalid-command'])
    .exit(127)
