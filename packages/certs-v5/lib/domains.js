...
function printDomains(domainsTable, message) {
  ...
  if (domainsTable.length === 0) {
    cli.styledHeader(`${message}  Add a custom domain to your app by running ${cli.color.app('heroku domains:add <yourdomain.com>')}`)
    /* eslint-enable no-irregular-whitespace */
  } else {
    cli.styledHeader(`${message}  Update your application's DNS settings as follows`)
