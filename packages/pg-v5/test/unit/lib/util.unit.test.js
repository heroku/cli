...
describe('util', () => {
  describe('parsePostgresConnectionString', () => {
    ...
    it('correctly identifies essential plans', () => {
      ...
      expect(parsed(addon("heroku-postresql:mini"))).to.equal(true)
      expect(parsed(addon("heroku-postresql:basic"))).to.equal(true)
      expect(parsed(addon("heroku-postresql:essential-0"))).to.equal(true)
      expect(parsed(addon("heroku-postresql:standard-0"))).to.equal(false)
      expect(parsed(addon("heroku-postresql:private-0"))).to.equal(false)
      expect(parsed(addon("heroku-postresql:shield-0"))).to.equal(false)
    })
  })
})
