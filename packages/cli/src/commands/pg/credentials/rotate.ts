...
export default class Rotate extends Command {
    ...
    static flags = {
      name: flags.string({char: 'n', description: 'which credential to rotate (default credentials if not specified)'}),
      all: flags.boolean({description: 'rotate all credentials'}),
    ...
    public async run(): Promise<void> {
      ...
      const body = {hostname: host(), forced: force ?? undefined}
      if (all) {
        ux.action.start(`Rotating all credentials on ${color.yellow(db.name)}`)
        await this.heroku.post(`/postgres/v0/databases/${db.name}/credentials_rotation`, body)
      } else {
        ux.action.start(`Rotating ${cred} on ${color.yellow(db.name)}`)
        await this.heroku.post(`/postgres/v0/databases/${db.name}/credentials/${encodeURIComponent(cred)}/credentials_rotation`, body)
      }
