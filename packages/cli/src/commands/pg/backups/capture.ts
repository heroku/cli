...
export default class Capture extends Command {
    ...
    static flags = {
      ...
      snapshot: flags.boolean({hidden: true}),
    ...
    public async run(): Promise<void> {
      ...
      const {app, snapshot, 'wait-interval': waitInterval, verbose} = flags
      const {database} = args
      ...
      if (snapshot) {
        ux.action.start(`Taking snapshot of ${color.yellow(db.name)}`)
        await this.heroku.post(`/postgres/v0/databases/${db.id}/snapshots`, {hostname: pgHost()})
        ux.action.stop()
      } else {
        try {
