...
import PromptCollection = inquirer.prompts.PromptCollection
...
const stubbedPrompt = sinon.stub(inquirer, 'prompt')
  .callsFake((questions: QuestionCollection<inquirer.Answers>) => {
    ...
    const promise = Promise.resolve(stubbedPromptReturnValue) as ReturnType<typeof inquirer.prompt>
    promise.ui = new PromptUI({} as PromptCollection)
    return promise
  })
...
describe('heroku certs:add', async () => {
  ...
  it('warns on deprecated --bypass flag', async () => {
    nock('https://api.heroku.com')
      .get('/apps/example')
      .reply(200, {space: null})
    mockDomains()
    ...
    mock.done()
    expect(stderr.output).to.include('use of the --bypass flag is deprecated.')
    expect(stderr.output).to.include('Adding SSL certificate to example... done')
  })
})
