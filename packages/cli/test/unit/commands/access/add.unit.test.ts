...
describe('heroku access:add', function () {
  context('with a team app with user permissions', function () {
    ...
    it('adds user to the app with permissions, and view is implicit', async function () {
      ...
      //   'gandalf@heroku.com',
      // ])
      const {stdout, stderr} = await runCommand(['access:add', '--app', 'myapp', '--permissions', 'deploy', 'gandalf@heroku.com'])
      // await runCommand(Cmd, [
      //   '--app',
      //   'myapp',
      //   '--permissions',
      ...
      apiPost.done()
      expect('').to.eq(stdout)
      expect(stderr).to.include('Adding gandalf@heroku.com access to the app myapp with deploy, view permissions... done')
    })
