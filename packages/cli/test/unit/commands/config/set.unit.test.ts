...
describe('config:set', function () {
  ...
  it('sets a config var', async () => {
    ...
    const {stdout, stderr} = await runCommand(['config:set', 'RACK_ENV=production', '--app', 'myapp'])
  ...
  it('sets a config var with an "=" in it', async () => {
    ...
    const {stdout, stderr} = await runCommand(['config:set', 'RACK_ENV=production=foo', '--app', 'myapp'])
  ...
  it('errors with invalid args', async () => {
