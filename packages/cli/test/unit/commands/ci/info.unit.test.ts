...
describe('ci:info', function () {
  ...
  it('errors when not specifying a test run', async () => {
  ...
  describe('when specifying a pipeline', function () {
    ...
    it('it shows the setup, test, and final result output', async () => {
    ...
    describe('and the exit was not successful', function () {
      ...
      it('it shows the setup, test, and final result output', async () => {
        ...
        const {stdout, error} = await runCommand(['ci:info', `${testRun.number}`, `--pipeline=${pipeline.name}`])
    ...
    describe('when the pipeline has parallel test runs enabled', function () {
      it('shows a result for each node', async () => {
        nock('https://api.heroku.com')
          .get(`/pipelines?eq[name]=${pipeline.name}`)
          .reply(200, [
            {id: pipeline.id},
      ...
      describe('and the user passes in a test node index', function () {
        it('displays the setup and test output for the specified node', async () => {
          nock('https://api.heroku.com')
            .get(`/pipelines?eq[name]=${pipeline.name}`)
            .reply(200, [
              {id: pipeline.id},
        ...
        describe('and the pipeline does not have parallel tests enabled', function () {
          it('displays the setup and test output for the first node and a warning', async () => {
            nock('https://api.heroku.com')
              .get(`/pipelines?eq[name]=${pipeline.name}`)
              .reply(200, [
                {id: pipeline.id},
            ...
            const {stdout, stderr} = await runCommand(['ci:info', `${testRun.number}`, `--pipeline=${pipeline.name}`, '--node=1'])
