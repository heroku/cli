...
describe('drains', function () {
  ...
  it('shows log drains', async () => {
    nock('https://api.heroku.com:443')
      .get('/apps/myapp/log-drains')
      .reply(200, [DRAIN])

    const {stdout, stderr} = await runCommand(['drains', '-a', 'myapp'])
...
`)
  ...
  it('shows drain_id for both', async () => {
    nock('https://api.heroku.com:443')
    ...
    const {stdout, stderr} = await runCommand(['drains', '-a', 'myapp', '--extended'])
...
`)
  ...
  it('shows correct json', async () => {
    nock('https://api.heroku.com:443')
      .get('/apps/myapp/log-drains?extended=true')
      .reply(200, EXTENDED_DRAINS)

    const {stdout, stderr} = await runCommand(['drains', '-a', 'myapp', '--extended', '--json'])
