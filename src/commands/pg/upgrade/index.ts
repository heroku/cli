...
export default class Upgrade extends Command {
  ...
  static description = heredoc(`
    We're deprecating this command. To upgrade your database's Postgres version, use the new ${color.code('pg:upgrade:*')} subcommands. See ${color.info('https://devcenter.heroku.com/changelog-items/3179')}.
  ...
  public async run(): Promise<void> {
    ...
    if (utils.pg.isLegacyEssentialDatabase(db))
      ux.error(`You can only use ${color.code('heroku pg:upgrade')} on Essential-tier databases and follower databases on Standard-tier and higher plans.`)
    ...
    if (replica.following) {
      ...
      await new ConfirmCommand().confirm(app, confirm, heredoc(`
        ...
        Destructive action
        You're upgrading ${color.datastore(db.name)} to ${versionPhrase}. The database will stop following ${color.datastore(origin)} and become writable.
      ...
      await new ConfirmCommand().confirm(app, confirm, heredoc(`
        ...
        Destructive action
        You're upgrading ${color.datastore(db.name)} to ${versionPhrase}.
