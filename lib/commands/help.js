...
class Help extends Command {
  async run () {
    plugins.load()
    let subject = this.argv.slice(1).find(arg => !['help', '-h', '--help'].includes(arg))
    if (!subject) return this.all()
    let topic = plugins.topics.find(t => t.name === subject.split(':')[0])
    if (!topic) return this.all()
    if (topic.help) return topic.help(subject)
    this.topic(topic, subject)
  }
  ...
  topic (topic, subject) {
    ...
    let command = subject.split(':').slice(1).join(':')
    command = plugins.commands.find(c => c.topic === topic.name && (c.command === command || (!c.command && !command)))
    ...
    let commands = plugins.commands.filter(c => c.topic === subject && c.command)
    if (commands.length === 0) return
    this.log(`${this.argv[0]} ${subject} commands: (${this.color.cmd(this.argv[0] + ' help ' + subject + ':COMMAND')} for details)
  `)
    let maxLength = max(commands, 'command.length').command.length + subject.length + 1
