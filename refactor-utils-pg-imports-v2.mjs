...
function processFile(filePath) {
  ...
  const importRegex = /import\s+({[^}]+}|\*\s+as\s+\w+)\s+from\s+'@heroku\/heroku-cli-util'/g;
  ...
  // If we have utils or need pg types, process this file
  if (importsNeeded.has('utils') || typeImportsNeeded.has('pg')) {
    ...
    // Find where to insert the new imports (after other heroku imports)
...
console.log('Starting migration of utils/pg imports...');
